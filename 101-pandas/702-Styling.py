...
#One can apply conditional formatting, the visual styling of a DataFrame depending on the data within, 
#by using the DataFrame.style property. 
#This is a property that returns a Styler object, which has useful methods for formatting and displaying DataFrames.

#The styling is accomplished using CSS. 
#You write “style functions” that take scalars, DataFrames or Series, and 
#return like-indexed DataFrames or Series with CSS "attribute: value" pairs for the values. 
...
#Both of those methods take a function (and some other keyword arguments) and 
#applies your function to the DataFrame in a certain way. 
#Styler.applymap works through the DataFrame elementwise. 
#Styler.apply passes each column or row into your DataFrame one-at-a-time or 
#the entire table at once, depending on the axis keyword argument. 
...
#Example of usual rendering a DataFrame, without any (visible) styles:
df.style

#Note: The DataFrame.style attribute is a property that returns a Styler object. 
#Styler has a _repr_html_ method defined on it so they are rendered automatically. 
...
df.style.render()
#The above output looks very similar to the standard DataFrame HTML representation. 
#But we’ve done some work behind the scenes to attach CSS classes to each cell. 
#We can view these by calling the .render method.
df.style.highlight_null().render().split('\n')[:10]

#The row0_col2 is the identifier for that particular cell. 
#We’ve also prepended each row/column identifier with a UUID unique to each DataFrame 
#so that the style from one doesn’t collide with the styling from another within the same notebook or 
#page (you can set the uuid if you’d like to tie together the styling of two DataFrames).
...
#Notice the similarity with the standard df.applymap, which operates on DataFrames elementwise. 
#We want you to be able to reuse your existing knowledge of how to interact with DataFrames.
#Notice also that our function returned a string containing the CSS attribute and value, 
#separated by a colon just like in a <style> tag. This will be a common theme.

#Finally, the input shapes matched. 
#Styler.applymap calls the function on each scalar input, and the function returns a scalar output.
...
#Instead, we’ll turn to .apply which operates columnwise (or rowwise using the axis keyword). 
#Later on we’ll see that something like highlight_max is already defined on Styler so you wouldn’t need to write this yourself.
def highlight_max(s):
    '''
...
s
/Users/taugspurger/miniconda3/envs/travis-36-doc/lib/python3.6/site-packages/matplotlib/colors.py:504: RuntimeWarning: invalid value encountered in less
  xa[xa < 0] = -1
Out[18]:
A	B	C	D	E
0	1	1.32921	nan	-0.31628	-0.99081
