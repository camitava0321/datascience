...
#%%
#3. Normalization
#Normalization is the process of scaling data such that it falls between a certain interval. 
#For example, we can scale a feature such that the data it contains falls under 0 and 1.

#3.1 Min-max normalization
#Min-max normalization is a widely used normalization technique. 
#Min-max normalization utilizes the min and max of the feature to scale the data.
...
x_train_scaled = scaler.fit_transform(x_train)
#%%4. Data Reduction
#Data reduction is the process of reducing the data along the column axis. 
#We perform data reduction techniques to avoid the overfitting phenomenon. 
#Overfitting is a phenomenon in which the model would show high performance in training 
#but fails to generalize to the new unseen data. 

#One of the dimensionality reduction techniques is feature selection which discards a 
#feature after looking at the importance of the feature statistically. 
#There are a wide variety of feature selection techniques available like 
#RFE (Recursive Feature Elimination), 
#RFECV (Recursive Feature Elimination with Cross Validation), and 
#Sequential Feature Selector.

#RFE (Recursive Feature Elimination)
#Recursive Feature Elimination is a type of wrapper feature selection that requires a 
#model to find the importance of the feature. 
#As the name says, It recursively eliminates the feature according to the 
#importance of the feature.
 
#The setback of using RFE is that we have to provide a number of features to select. 
#To overcome this we can use RFECV which uses cross-validation to determine the number of 
#features to select. 

#In the code below, the x_train must be preprocessed before using it.
...
#Recursive Feature Elimination with Cross Validation is a type of wrapper feature selection 
#that is very similar to RFE but eliminates the hyperparameter n_features_to_select.
#It recursively eliminates the features and uses cross-validation to determine the 
...
#%%
#Data Modeling
#Data modeling is the process of training a model that learns the relationship between the 
#features and the target. 
#A large amount of data is preferable because more the data more are the patterns to learn. 
#The trained model is then used to predict the new unseen model. 
#A highly complex and flexible model like Decision-Tree is more prone to overfit the data.
#To avoid overfitting we can use ensemble methods. 
#Bagging is the process of combining several weak models to produce an efficient model but 
#the Boosting algorithms make use of several weak models and feed each of the weak model’s 
#output to the next weak model thereby producing an efficient model.

#An example of a bagging algorithm is the Random Forest algorithm and an example of a 
#boosting algorithm is the Gradient Boosting algorithm. 
#We can also use cross-validation techniques to select the best model. 
 
#Performance metrics for Supervised Machine Learning Model
#Performance metrics are used to assess the quality of the trained model. 
#In regression model training is referred to as learning the parameters of the model, 
#so measuring the performance means, how well the model has learned the parameters. 
#To know the performance we use cost functions that compare the predicted and actual values.

#Regression and classification techniques require a separate set of metrics to 
#analyze the performance of the model. Gradient descent, RMS Prop, 
#and Adam are cost function optimization techniques to optimize the model. 
#The gradient descent optimization algorithm finds the optimal parameters of the model by 
#finding the error gradients. 
#We can run the algorithm for many cycles until we arrive at a globally optimal solution.
...
#What is Flask
#Flask is a web application framework written in python to build web applications and 
#API architecture. 
#We have utilized flask to develop an API for training and testing the titanic dataset.
...
#First of all, the code defines a preprocessing pipeline, in our case, 
#it is a pipeline that consists of Median imputation, Mode imputation, Robust Scaler, 
#and Frequency encoding. The median imputer class imputes the missing values in the 
#numerical columns with the median of those columns and the mode imputer imputes the missing 
#values in categorical columns with the mode (the most frequent category) of those columns 
#and Robust scaler standardizes the data such that the mean is zero and the standard deviation 
#is one with a more robust way that is not influenced by the outliers and the frequency encoder 
#encodes the categorical data by mapping the categories with the frequencies of those categories.
#When the fit_transform method is invoked the above pipeline would be executed. 
#At first, the pipeline code splits the data into numeric and categoric columns and applies 
#appropriate preprocessing respectively. After performing/running a pipeline object we must 
#pickle the pipeline object to utilize it for transforming the test data. 
#The methodology used here to write preprocessors is also called the custom transformers, 
#by taking advantage of the custom transformers you can give additional functionality to the data.

#After running the pipeline and preprocessing the data we train different models on the 
#preprocessed data and check the accuracy. We have used Logistic Regression, 
#Gradient Boosting classifier, and Random Forest classifier to train. 
#In our case, the Random forest classifier performed better with an accuracy of 98%.
#After training the model, we have to pickle the model to predict it on the future data. 
#Pickling is a serialization technique that persists the model’s state and behavior.
